...
impl Grid {
    ...
    fn Add (&mut self, transform: Transform, crow: Crow) {
        ...
        let x = (transform.translation.x.abs() + if transform.translation.x < 0.0 {0.0} else {self.size as f32 / 2.0}) as usize / self.size ;
        let y = (transform.translation.y.abs() + if transform.translation.y < 0.0 {0.0} else {self.size as f32 / 2.0}) as usize / self.size ;
        let z = (transform.translation.z.abs() + if transform.translation.z < 0.0 {0.0} else {self.size as f32 / 2.0}) as usize / self.size ;
